...
export const NIP46_PERMS =
  ...
  [CLIENT_AUTH, AUTH_JOIN, MESSAGE, THREAD, COMMENT, GROUPS, WRAP, REACTION].map(k => `sign_event:${k}`).join(",")
...
export const pubkeyLink = (pubkey: string, relays = ctx.app.router.FromPubkeys([pubkey]).getUrls()) =>
...
export const deriveEvent = (idOrAddress: string, hints: string[] = []) => {
  ...
  return derived(deriveEvents(repository, {filters, includeDeleted: true}), (events: TrustedEvent[]) => {
...
export const hasNip29 = (relay?: Relay) => relay?.profile?.supported_nips?.map(String)?.includes("29")
...
export const roomComparator = (url: string) => (room: string) => displayChannel(url, room).toLowerCase()
...
export const userSettingValues = withGetter(derived(userSettings, $s => $s?.values || defaultSettings))
