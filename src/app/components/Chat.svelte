<script lang="ts">
  ...
  import {pubkey, tagPubkey, formatTimestampAsDate, inboxRelaySelectionsByPubkey, load} from "@welshman/app"
  ...
  const showMembers = () => pushModal(ProfileList, {pubkeys: others, title: `People in this conversation`})
      ...
      {#snippet title()}
        <div class="flex flex-col gap-1 sm:flex-row sm:gap-2">
          ...
          {:else}
            ...
            {#if others.length > 2}
              <Button onclick={showMembers} class="btn btn-link hidden sm:block">Show all members</Button>
            {/if}
          {/if}
        </div>
      {/snippet}
    ...
    {#if missingInboxes.includes($pubkey!)}
      <div class="py-12">
        <div class="card2 col-2 m-auto max-w-md items-center text-center">
          ...
          <p>
            In order to deliver messages, {PLATFORM_NAME} needs to know where to send them. Please visit your <Link
              class="link"
              href="/settings/relays">relay settings page</Link
    ...
    {:else if missingInboxes.length > 0}
      <div class="py-12">
        <div class="card2 col-2 m-auto max-w-md items-center text-center">
          ...
          <p>
            In order to deliver messages, {PLATFORM_NAME} needs to know where to send them. Please make sure everyone in
            this conversation has set up their inbox relays.
          </p>
        </div>
      </div>
    {/if}
