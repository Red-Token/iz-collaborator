...
<script lang="ts">
  ...
  import {pubkey, formatTimestampAsDate, inboxRelaySelectionsByPubkey, load, tagPubkey} from "@welshman/app"
  ...
  const missingInboxes = derived(inboxRelaySelectionsByPubkey, $m => pubkeys.filter(pk => !$m.has(pk)))
    ...
    <PageBar>
      <div slot="title" class="flex flex-col gap-1 sm:flex-row sm:gap-2">
        ...
        {:else}
          ...
          {#if others.length > 2}
            <Button on:click={showMembers} class="btn btn-link hidden sm:block">Show all members</Button>
          {/if}
        {/if}
      </div>
      <div slot="action">
