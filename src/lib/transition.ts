...
export function slideAndFade(node: any, {delay = 0, duration = 400, easing = cubicOut, axis = "y"} = {}) {
  ...
  const capitalized_secondary_properties = secondary_properties.map((e: string) => `${e[0].toUpperCase()}${e.slice(1)}`)
...
export const conditionalTransition = (condition: any, transition: any) => (node: any, args?: any) =>
