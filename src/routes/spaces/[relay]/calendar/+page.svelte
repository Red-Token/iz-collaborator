<script lang="ts">
  ...
  $effect(() => {
    ...
    if (initialScrollDone) {
      ...
      const {offsetTop, clientHeight} = document.querySelector(".calendar-event-" + event.id) as HTMLElement
  ...
  onMount(() => {
    ...
    const subscriptionFilters = [{kinds: [DELETE, REACTION, EVENT_TIME], "#h": [GENERAL], since: now()}]
